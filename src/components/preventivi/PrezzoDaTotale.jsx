...
// Gli articoli a prezzo fisso o "a pezzo" non vanno riscalati: il loro
// prezzo non dipende dai metri quadri, quindi restano come sono e il
// loro importo viene sottratto prima di spalmare il resto.
const siScalaSuiMq = (item) => {
  const calcType = item.rawInput?.calcType || item.complementoCalcType;
  if (calcType === 'fisso' || calcType === 'pz') return false;
...
export default function PrezzoDaTotale({ items, sconto, iva, onApplica }) {
  ...
  const target = Number(String(totaleDesiderato).replace(',', '.')) || 0;
  ...
  const scontoFrazione = 1 - (Number(sconto) || 0) / 100;
  if (scontoFrazione > 0) imponibileTarget = imponibileTarget / scontoFrazione;

  const daSpalmare = imponibileTarget - importoFisso;
  const prezzoAlMq = mqTotali > 0 && daSpalmare > 0 ? daSpalmare / mqTotali : null;
    ...
    const aggiornati = (items || []).map(item => {
      ...
      return {
        ...
        rawInput: item.rawInput
          ? { ...item.rawInput, unitPrice: nuovoPrezzo, basePrice: Number(prezzoAlMq.toFixed(2)) }
          : item.rawInput
      };
    });
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mt-6">
      ...
      {target > 0 && mqTotali > 0 && daSpalmare <= 0 && (
        <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">
